...
struct Image {
    ...
    int tex_width = 0;
    int tex_height = 0;
    Image(char* image_path, GLFWwindow* window) {
         //rendering commands hee 
        float vertices[] = {
            // positions             // texture coords
            1.0f,  1.0f, /**/ 1.0f, 1.0f, // top right
            1.0f, -1.0f, /**/ 1.0f, 0.0f, // bottom right
            -1.0f, -1.0f,/**/ 0.0f, 0.0f, // bottom leftI
        ...
        scale = calc_natural_scale(window);
    ...
    float calc_natural_scale(GLFWwindow *window) {
        int screen_width, screen_height;
        glfwGetWindowSize(window ,&screen_width,&screen_height);
        float fin_scale_x = ((float)this->tex_width/screen_width);
        float fin_scale_y = ((float)this->tex_height/screen_height);
        return 1/(fin_scale_x > fin_scale_y ? fin_scale_x : fin_scale_y);
    }


    void apply_transformations(GLFWwindow *window) {
        int screen_width, screen_height;
        glfwGetWindowSize(window ,&screen_width,&screen_height);
    ...
    void render(GLFWwindow* main_window) {
        ...
        apply_transformations(main_window);
...
int main(int argc, char** argv) {
    ...
    Image picture(argv[1],  main_window);
...
void processInput(GLFWwindow *window, Image *picture){
    ...
    if(glfwGetKey(window, GLFW_KEY_0) == GLFW_PRESS) {
        picture->displace_x = 0.0f;
        picture->displace_y = 0.0f;
        picture->scale = picture->calc_natural_scale(window);
